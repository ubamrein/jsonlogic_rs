...
pub fn compute(args: &[Expression], data: &Data) -> Value {
    ...
    if let Some(val) = logic::not_before(&a, &b) {
        Value::Bool(val)
    } else {
        Value::Null
    }
...
mod tests {
    ...
    #[test]
    fn test() {
        assert_eq!(json!(true), compute_const!(json!("2020-01-02"), json!("2020-01-01")));
        assert_eq!(json!(true), compute_const!(json!("2020-01-02"), json!("2020-01-02")));
        assert_eq!(json!(false), compute_const!(json!("2020-01-01"), json!("2020-01-02")));
    }
}
