...
pub fn compute(args: &[Expression], data: &Data) -> Value {
    ...
    let date = match unit.as_str() {
        "year" | "years" => date.sub(chrono::Duration::weeks(offset * 52)),
        "month" | "months" => date.sub(chrono::Duration::weeks(offset * 4)),
        "week" | "weeks" => date.sub(Duration::weeks(offset)),
        "day" | "days" => date.sub(Duration::days(offset)),
        "hour" | "hours" => date.sub(Duration::hours(offset)),
        "minute" | "minutes" => date.sub(Duration::minutes(offset)),
        "second" | "seconds" => date.sub(Duration::seconds(offset)),
        _ => return Value::Null,
    };
...
mod tests {
    ...
    fn test_subtraction() {
        ...
        assert_eq!(
            compute_const!(
                Value::String("2021-01-02T00:00:00.000".to_string()),
                Value::Number(1.into()),
                Value::String("day".to_string())
            ),
            timestamp
        ...
        assert_eq!(
            compute_const!(
                Value::String("2021-01-01T01:00:00.000Z".to_string()),
                Value::Number(1.into()),
